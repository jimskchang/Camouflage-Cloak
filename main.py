...
from src.settings import MAC  # Dynamically retrieved MAC address
...
def collect_fingerprint(target_host, dest, nic):
    logging.info(f"📡 Starting OS Fingerprinting on target: {target_host}")
    logging.info(f"🌐 Listening on interface: {nic}")
    logging.info(f"💾 Saving fingerprints to: {dest}")
    logging.info(f"🔧 Using MAC: {MAC}")
    ...
    logging.info(f"✅ OS Fingerprinting Complete. Captured {packet_count} packets.")
...
def convert_to_json(file_path):
    try:
        ...
        logging.info(f"✅ Converted {file_path} to base64-encoded JSON.")
...
def main():
    ...
    parser.add_argument("--dest", default=DEFAULT_OS_RECORD_PATH, help="Directory to store fingerprint data")
    parser.add_argument("--os", help="OS name to mimic for deception (for --od)")
    ...
    if args.scan == "ts":
        ...
        logging.info(f"🎭 Starting OS Deception for {args.os}")
