...
def main():
    ...
    parser.add_argument('--os_record-dir', action="store", help='Specify output directory for storing OS records')
    ...
    if args.scan:
        ...
        if port_scan_tech == 'ts':
            ...
            deceiver.os_record(output_path=args.os_record_dir)
        elif port_scan_tech == 'od':
            if args.os is None:
                logging.debug('No os is designated')
            else:
                deceiver = OsDeceiver(args.host, args.os)
                deceiver.os_deceive(output_path=args.os_record_dir)
        elif port_scan_tech == 'rr':
            deceiver = OsDeceiver(args.host, args.os)
            deceiver.store_rsp(output_path=args.os_record_dir)

        if args.status:
            deceive_status = args.status
            if port_scan_tech == 'pd':
                deceiver = PortDeceiver(args.host)
                deceiver.deceive_ps_hs(deceive_status, output_path=args.os_record_dir)
