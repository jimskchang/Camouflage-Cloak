...
def synthesize_response(pkt, template: bytes, ttl: int = 64, window: int = 8192, deceiver=None) -> bytes:
    try:
        ...
        if IP in scapy_pkt:
            ...
            # Support for TOS/ECN if available
            ...
            # DF bit
            df = deceiver.os_flags.get("df", False) if deceiver else False
            scapy_pkt[IP].flags = 'DF' if df else 0
        ...
        if TCP in scapy_pkt:
            ...
            if deceiver:
                ...
                scapy_pkt[TCP].options = deceiver.get_tcp_options(pkt.src_ip, ts_echo)
