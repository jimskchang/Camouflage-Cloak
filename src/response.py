...
def synthesize_response(pkt, template: bytes, ttl: int = 64, window: int = 8192, deceiver=None) -> bytes:
    try:
        ...
        if IP in scapy_pkt:
            ...
            scapy_pkt[IP].id = deceiver.get_ip_id(pkt.src_ip) if deceiver else 0
            scapy_pkt[IP].flags = 'DF' if deceiver and deceiver.os_template.get("df", False) else 0
            if deceiver:
                scapy_pkt[IP].tos = deceiver.os_template.get("tos", 0)
        ...
        if TCP in scapy_pkt:
            ...
            scapy_pkt[TCP].ack = pkt.l4_field.get("seq_num", 0) + 1
            scapy_pkt[TCP].flags = "SA"  # SYN+ACK default
            scapy_pkt[TCP].window = window

            if deceiver:
                ts_echo = getattr(pkt, 'tcp_options', {}).get("TSval", 0)
