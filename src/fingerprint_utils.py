...
def gen_dns_key(packet: bytes):
    try:
        ip_header = packet[14:34]
        udp_header = packet[34:42]
        dns_data = packet[42:50]
        ip_key = ip_header[:8] + b'\x00' * 8
        key = ip_key + udp_header + dns_data
        return key, hashlib.sha256(key).digest()
    except Exception as e:
        logging.warning(f"⚠️ gen_dns_key failed: {e}")
        return b'', None
