...
class OsDeceiver:
    def __init__(self, target_host: str, target_os: str, dest: Optional[str] = None):
        """
    ...
    def load_file(self, pkt_type: str) -> Dict[str, Any]:
        ...
        try:
            with open(file_path, 'r') as file:
                ...
                return json.loads(packet_data) if packet_data else {}
    ...
    def store_rsp(self):
        """
        Store responses to specific TCP ports for deception.
        """
        rsp = {}
        while True:
    ...
    def os_deceive(self):
        ...
        while True:
            ...
            if proc == 'tcp' and pkt.l3_field.get('dest_IP') == Packet.ip_str2byte(self.host):
                if pkt.l4_field.get('dest_port') in settings.FREE_PORT:
...
def deceived_pkt_synthesis(proc: str, req: Packet, template: Dict[str, Any]) -> Optional[bytes]:
    """
    Generate a deceptive packet based on the request and template.
    """
    key, _ = gen_key(proc, req.packet)
    raw_template = template.get(proc, {}).get(key)
    if not raw_template:
        return None
    ...
    common_updates = {
        'dMAC': req.l2_field.get('sMAC'),
        'sMAC': req.l2_field.get('dMAC'),
        'src_IP': req.l3_field.get('dest_IP'),
        'dest_IP': req.l3_field.get('src_IP')
    }
    template_pkt.l2_field.update(common_updates)
    template_pkt.l3_field.update(common_updates)

    if proc == 'tcp':
        template_pkt.l4_field.update({'src_port': req.l4_field.get('dest_port'), 'dest_port': req.l4_field.get('src_port')})
    elif proc == 'icmp':
        template_pkt.l4_field.update({'ID': req.l4_field.get('ID'), 'seq': req.l4_field.get('seq')})
    elif proc == 'arp':
        template_pkt.l3_field.update({
            'sender_mac': settings.mac,
            'sender_ip': socket.inet_aton(settings.host),
            'recv_mac': req.l3_field.get('sender_mac'),
            'recv_ip': req.l3_field.get('sender_ip')
        })

    template_pkt.pack()
    return template_pkt.packet

def gen_key(proc: str, packet: bytes) -> tuple:
    """
    Generate a key based on protocol type for template matching.
    """
    return {
        'tcp': gen_tcp_key,
        'udp': gen_udp_key,
