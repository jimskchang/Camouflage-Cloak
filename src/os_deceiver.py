...
class OsDeceiver:
    def __init__(self, target_host: str, target_os: str, dest=None, nic: str = None):
        ...
        self._init_plot()
        logging.info(f"🛡️ OS Deceiver initialized for '{self.os}' on {self.nic} ({self.host}, {self.mac})")
    ...
    def _update_plot(self, frame):
        ...
        values = [self.protocol_stats.get(k, 0) for k in labels]
    ...
    def os_deceive(self, timeout_minutes: int = 5):
        ...
        while datetime.now() < timeout:
            try:
                ...
                # --- Check CUSTOM_RULES ---
                if self.apply_custom_rules(proto, pkt, flags, tos, frag, dst_port, src_ip):
                    continue  # already handled by rule
                ...
                if not template:
                    for k in templates.get(proto, {}):
                        if key.startswith(k[:16]):
                            ...
                            logging.debug(f"🔍 Fuzzy matched {proto.upper()} template")
                            break

                if template:
                    if random.random() < 0.05:
                        logging.debug("🎲 Random drop (5%)")
                        continue
    ...
    def apply_custom_rules(self, proto, pkt, flags, tos, frag, dst_port, src_ip):
        for rule in CUSTOM_RULES:
            match = rule.get("proto", "").lower() == proto
            if "port" in rule:
                match &= rule["port"] == dst_port
            if "flags" in rule and proto == "tcp":
                rule_flags = rule["flags"]
                match &= all(f in pkt.l4_field.get("flags_str", "") for f in rule_flags)
            if "type" in rule and proto == "icmp":
                match &= rule["type"] == pkt.l4_field.get("icmp_type")
            if "dscp" in rule:
                match &= rule["dscp"] == (tos >> 2)
            if "fragmented" in rule:
                match &= rule["fragmented"] == (frag != 0)
            if "src_ip" in rule:
                match &= rule["src_ip"] == src_ip
            ...
            if match:
                ...
                if action == "drop":
                    return True
                elif action == "rst" and proto == "tcp":
                    self.send_tcp_rst(pkt)
                    return True
                elif action == "icmp_unreachable" and proto == "udp":
                    self.send_icmp_port_unreachable(pkt)
                    return True
    ...
    def send_tcp_rst(self, pkt):
        ether = Ether(src=self.mac, dst=pkt.l2_field["sMAC"])
    ...
    def send_icmp_port_unreachable(self, pkt):
        ...
        reply = ether / ip / icmp / bytes(inner)[:28]
